"""LightGBM 模型版本登记 — ``va_lgb_models`` 表的 CRUD。
...
与 limit-up-board 的同名表相比，VA 额外记录 ``label_threshold_pct`` /
``label_source`` 两列，以便不同标签语义的模型不被混用（VA 的标签可在
5d-max / 3d / 10d-max 三种来源 + 任意阈值之间切换）。
...
_SELECT_ALL_COLS = (
    ...
    "label_threshold_pct, label_source, "
...
def insert_model(db: Database, record: ModelRecord, *, activate: bool = True) -> None:
    ...
    with db.transaction():
        if activate:
            db.execute("UPDATE va_lgb_models SET is_active = FALSE WHERE is_active = TRUE")
        db.execute(
            "INSERT INTO va_lgb_models("
            "model_id, schema_version, train_start_date, train_end_date, "
            "n_samples, n_positive, cv_auc_mean, cv_auc_std, cv_logloss_mean, "
            "feature_count, feature_list_json, hyperparams_json, "
            "label_threshold_pct, label_source, "
...
def set_active(db: Database, model_id: str) -> bool:
    ...
    with db.transaction():
        row = db.fetchone("SELECT 1 FROM va_lgb_models WHERE model_id = ?", (model_id,))
        if row is None:
            return False
        db.execute("UPDATE va_lgb_models SET is_active = FALSE WHERE is_active = TRUE")
        db.execute("UPDATE va_lgb_models SET is_active = TRUE WHERE model_id = ?", (model_id,))
        return True


def deactivate_all(db: Database) -> None:
    """Force all rows to is_active = FALSE (operational kill-switch)."""
    db.execute("UPDATE va_lgb_models SET is_active = FALSE")


def list_models(db: Database) -> list[ModelRecord]:
    rows = db.fetchall(
        f"SELECT {_SELECT_ALL_COLS} FROM va_lgb_models ORDER BY created_at DESC"
    )
...
def get_model(db: Database, model_id: str) -> ModelRecord | None:
    row = db.fetchone(
        f"SELECT {_SELECT_ALL_COLS} FROM va_lgb_models WHERE model_id = ?",
        (model_id,),
    )
...
def get_active(db: Database) -> ModelRecord | None:
    row = db.fetchone(
        f"SELECT {_SELECT_ALL_COLS} FROM va_lgb_models "
...
def delete_model(db: Database, model_id: str) -> bool:
    ...
    with db.transaction():
        existing = db.fetchone("SELECT 1 FROM va_lgb_models WHERE model_id = ?", (model_id,))
        if existing is None:
            return False
        db.execute("DELETE FROM va_lgb_models WHERE model_id = ?", (model_id,))
        return True
...
def ensure_unique_model_id(db: Database, base_id: str) -> str:
    ...
    if db.fetchone("SELECT 1 FROM va_lgb_models WHERE model_id = ?", (base_id,)) is None:
        return base_id
    n = 2
    while True:
        candidate = f"{base_id}-{n}"
        if db.fetchone("SELECT 1 FROM va_lgb_models WHERE model_id = ?", (candidate,)) is None:
            return candidate
        n += 1
        if n > 999:
