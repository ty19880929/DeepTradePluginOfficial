...
与 :mod:`registry` 的 prune（日常维护：保留最近 N 个 + active）不同，
``cleanup.purge_lgb_artifacts`` 是 "scorched earth"——按调用方指定的范围
彻底清空。所有 destructive 行为通过本模块的 dataclass-style 参数显式表达，
避免 CLI 层直接散落 ``DELETE FROM`` / ``unlink`` 调用。
...
* 文件删除按"尽力而为"逐文件单独 try/except，统计实际删除的数量；磁盘
  权限错误不阻塞剩余清理。
* 返回 :class:`PurgeReport` 让 CLI / 测试两侧都能确认副作用范围。
"""
...
def _count_checkpoint_artifacts() -> tuple[int, int]:
    """``(n_fingerprint_dirs, n_day_shards)`` under ``checkpoints/``."""
    root = lgb_paths.checkpoints_dir()
    if not root.is_dir():
        return 0, 0
    n_dirs = 0
    n_shards = 0
    for sub in root.iterdir():
        ...
        days_dir = sub / "days"
        if days_dir.is_dir():
            n_shards += sum(1 for f in days_dir.glob("*.parquet") if f.is_file())
    return n_dirs, n_shards


def _purge_checkpoints() -> tuple[int, list[str]]:
    """Remove every fingerprint subdir under ``checkpoints/``. Returns (n_dirs, errors)."""
    root = lgb_paths.checkpoints_dir()
    if not root.is_dir():
        return 0, []
    n = 0
    ...
    for sub in list(root.iterdir()):
        ...
        try:
            ...
            errs.append(f"rmtree {sub.name}: {e}")
    return n, errs
...
def count_artifacts(db: Database) -> PurgeReport:
    ...
    n_ck, n_shards = _count_checkpoint_artifacts()
    report.n_checkpoint_dirs = n_ck
    report.n_checkpoint_shards = n_shards
    report.n_model_rows = _safe_count(db, "SELECT COUNT(*) FROM va_lgb_models")
    report.n_prediction_rows = _safe_count(
        db, "SELECT COUNT(*) FROM va_lgb_predictions"
    )
    return report
...
def purge_lgb_artifacts(
    ...
    *,
    ...
    models
        ...
        ``va_lgb_models``. After this the scorer enters the
        "no_active_model" branch on the next analyze run.
    predictions
        Truncate ``va_lgb_predictions`` (per-run scoring audit history).
    checkpoints
    ...
    try:
        with db.transaction():
            if models:
                row = db.fetchone("SELECT COUNT(*) FROM va_lgb_models")
                report.n_model_rows = int((row[0] if row else 0) or 0)
                db.execute("DELETE FROM va_lgb_models")
            if predictions:
                row = db.fetchone("SELECT COUNT(*) FROM va_lgb_predictions")
                report.n_prediction_rows = int((row[0] if row else 0) or 0)
                db.execute("DELETE FROM va_lgb_predictions")
    ...
    if checkpoints:
        ...
        _, n_shards = _count_checkpoint_artifacts()
        n_ck, errs = _purge_checkpoints()
        report.n_checkpoint_dirs = n_ck
        report.n_checkpoint_shards = n_shards
        report.errors.extend(errs)
