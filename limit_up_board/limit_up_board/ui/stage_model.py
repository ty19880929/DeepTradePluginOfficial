...
of 5 steps. ``STEP_STARTED`` adds (or reuses) a stage; ``STEP_FINISHED``
closes it; ``LLM_BATCH_FINISHED`` ticks progress; ``VALIDATION_FAILED``
records a failed batch on whichever stage is currently running. The
ordering of stages in the dashboard matches the order they were first
``push``-ed, which mirrors pipeline emit order.
...
class StageStatus(str, Enum):
    ...
    ``SKIPPED`` is for conditional stages (Step 4.5 single-batch case).
    """
...
class StageStack:
    ...
    def set_running(
        self, stage_id: str, *, total: int | None = None, now: float | None = None
    ) -> None:
    ...
    def mark_finished(
        self, stage_id: str, *, partial: bool = False, now: float | None = None
    ) -> None:
        st = self._index.get(stage_id)
        if st is None:
            return
