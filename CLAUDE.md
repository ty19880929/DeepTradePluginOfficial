...
- **Tushare API cache classification**: the framework (`deeptrade.core.tushare_client.API_CACHE_CLASS`) curates four cache buckets — `static` / `trade_day_immutable` / `trade_day_mutable` / `hot_or_anns`. APIs **not** in that table fall back to `trade_day_immutable` and emit a one-shot INFO log asking the plugin author to declare it explicitly. Whenever you add a new `tushare.call("<api>")` to a plugin, check whether `<api>` is already in the framework's table; if not, declare it under `permissions.tushare_apis.cache_overrides: { <api>: <class> }` in that plugin's yaml. Examples already present: `cyq_perf`/`index_daily` → `trade_day_immutable` (lub, apw); `namechange` → `static` (checkmate).
